...
import { Plus, List, Bot, Download, X } from "lucide-react";
      ...
      <div className="glass-card rounded-xl p-0.5 flex gap-0.5">
        {SUBTABS.map(({ id, label, Icon }) => (
          <button
            ...
            className={`relative flex-1 flex flex-col items-center gap-0.5 py-2 rounded-lg text-xs font-medium transition-all ${subTab === id ? "active-pill" : "text-muted"}`}
          >
            <Icon className="w-3.5 h-3.5" />
            {label}
            {id === "bot" && errorCount > 0 && (
              <span className="absolute top-1 right-2 w-3.5 h-3.5 rounded-full bg-red-500 text-[8px] font-bold text-white flex items-center justify-center">
                {errorCount}
              </span>
            )}
          </button>
